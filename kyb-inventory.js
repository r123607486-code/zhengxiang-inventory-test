...
// ------------------------------------------------------------
// 叫貨流程：與輪胎共用同一台購物車（order-cart.js），可以混放後一次送出。
// ============================================================
...
// 加入購物車：只決定車型、儲位、數量與金額，客戶資料在結帳時填。
function openKybOrderModal(itemId){
  ...
  const html=`<div class="sheet-head"><h2>加入購物車：${escapeHtml(item.carModel)}</h2><button class="sheet-close" onclick="closeModal()">✕</button></div><div class="form-row"><label>車型／品牌</label><input type="text" value="${escapeHtml(item.carModel)}（KYB）" disabled></div><div class="form-row"><label>庫存狀態</label><input type="text" value="實際庫存 ${physical}　已預留 ${reserved}　可用 ${available}" disabled></div><div class="form-row"><label>選擇儲位</label><select id="kybOrderLoc">${options.length?options.map((o,i)=>`<option value="${i}">${escapeHtml(o.code)}（庫存 ${o.qty}／可用 ${o.available}）</option>`).join(""):`<option value="">目前沒有可用庫存</option>`}</select></div><div class="form-row"><label>數量</label><select id="kybOrderQty"></select></div><section class="sales-pricing-box" id="kybOrderPriceBox" data-sales-source="kyb"><div class="sales-pricing-title">銷售金額與稅別</div><div class="form-row"><label>套用價目表</label><select id="kybOrderPricePriceList"><option value="">請先選擇品項</option></select></div><div class="form-row"><label>單價</label><input type="number" min="0" step="1" inputmode="numeric" id="kybOrderPriceUnitPrice" placeholder="請輸入實際成交單價"></div><div class="form-row"><label>稅別</label><select id="kybOrderPriceTaxMode"><option value="no_tax">不計稅</option><option value="tax_included">稅內含（5%）</option><option value="tax_excluded">稅外加（5%）</option></select></div><div class="sales-pricing-summary" id="kybOrderPriceSummary"></div></section><div class="note">客戶資料在購物車結帳時填一次就好；輪胎與 KYB 可以放在同一台車。</div><div class="form-actions"><button onclick="closeModal()">取消</button><button class="primary" id="kybOrderSubmitBtn">加入購物車</button></div>`;
  openModal(html);
  ...
  document.getElementById("kybOrderSubmitBtn").addEventListener("click",()=>{
    const opt=options[Number(document.getElementById("kybOrderLoc").value)],qty=Number(document.getElementById("kybOrderQty").value);
    ...
    addOrderCartLine({
      source:"kyb", itemId:item.id, itemLabel:`${item.carModel}（KYB）`,
      qty, loc:opt.code, batchDate:null, ...pricing
    });
    closeModal();
  });
}
